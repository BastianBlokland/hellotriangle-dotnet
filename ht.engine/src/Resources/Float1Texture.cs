...
{
    public sealed class Float1Texture : IInternalTexture
    {
        //Properties
        ...
        //Data
        private readonly ReadOnlyMemory<float> pixels; //stored row by row
        private readonly Int2 size;

        public Float1Texture(ReadOnlyMemory<float> pixels, Int2 size)
        {
            if (pixels.Length == 0)
                throw new ArgumentException($"[{nameof(Float1Texture)}] No pixels provided", nameof(pixels));
            if (pixels.Length != size.X * size.Y)
                throw new ArgumentException(
                    $"[{nameof(Float1Texture)}] Invalid count, expected: {size.X * size.Y}, got: {pixels.Length}", nameof(pixels));
            this.pixels = pixels;
            this.size = size;
        }
